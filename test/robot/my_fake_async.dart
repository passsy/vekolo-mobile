...
/// [MyFakeAsync.elapse] rather than the passing of real time.
///
/// The [`clock`][] property will be set to a clock that reports the fake
/// elapsed time. By default, it starts at the time [myFakeAsync] was created
...
T myFakeAsync<T>(T Function(MyFakeAsync async) callback, {DateTime? initialTime}) =>
...
class MyFakeAsync {
  ...
  /// The amount of fake time that's elapsed since this [MyFakeAsync] was
  /// created.
  Duration get elapsed => _elapsed;
  var _elapsed = Duration.zero;

  /// Whether Timers created by this FakeAsync will include a creation stack
  /// trace in [MyFakeAsync.pendingTimersDebugString].
  ...
  /// [myFakeAsync].
  int get periodicTimerCount => _timers.where((timer) => timer.isPeriodic).length;
  ...
  /// [`clock`]: https://www.dartdocs.org/documentation/clock/latest/clock/clock.html
  ///
  /// Note: it's usually more convenient to use [myFakeAsync] rather than creating
  /// a [MyFakeAsync] object and calling [run] manually.
  MyFakeAsync({DateTime? initialTime, this.includeTimerStackTrace = true}) {
  ...
  ///
  /// Any timers created within [run] or [myFakeAsync] will fire if their time is
  ...
  ///
  /// The [`clock`][] property will be set to a clock that reports the fake
  /// elapsed time. By default, it starts at the time the [MyFakeAsync] was
  ...
  /// Note: it's usually more convenient to use [myFakeAsync] rather than creating
  /// a [MyFakeAsync] object and calling [run] manually.
  T run<T>(T Function(MyFakeAsync self) callback) => runZoned(
    () => withClock(_clock, () => callback(this)),
    zoneSpecification: ZoneSpecification(
      createTimer: (_, __, zone, duration, callback) => _createTimer(duration, zone, callback, false),
      createPeriodicTimer: (_, __, zone, duration, callback) => _createTimer(duration, zone, callback, true),
      scheduleMicrotask: (_, parent, zone, microtask) {
        zone.parent!.scheduleMicrotask(microtask);
      },
    ),
  );
  ...
  Timer _createTimer(Duration duration, Zone zone, Function callback, bool periodic) {
    final timer = FakeTimer._(duration, zone, callback, periodic, this, includeStackTrace: includeTimerStackTrace);
    _timers.add(timer);
    return timer;
  }
...
/// An implementation of [Timer] that's controlled by a [MyFakeAsync].
///
...
class FakeTimer implements Timer {
  ...
  /// The [MyFakeAsync] instance that controls this timer.
  final MyFakeAsync _async;

  /// The value of [MyFakeAsync._elapsed] at (or after) which this timer should be
  /// fired.
  Duration _nextCall;

  /// The current stack trace when this timer was created.
  ///
  /// If [MyFakeAsync.includeTimerStackTrace] is set to false then accessing
  ...
  }) : duration = duration < Duration.zero ? (duration = Duration.zero) : duration,
